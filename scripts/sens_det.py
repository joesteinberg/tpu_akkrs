...
fnames = ['simul_agg_det0.csv','simul_agg_det1.csv','simul_agg_det2.csv']
...
lns=[]
for i in [0,1,2]:
    ln = axes.plot(df2[i].y,np.log(df2[i].exports),color=colors[i],alpha=0.7,label=slabs[i])
    lns=lns+ln
...
plt.savefig(outpath + 'simul_trans_sens_det.pdf',bbox_inches='tight')
plt.close('all')
...
lns=[]
for i in [0,1,2]:
    ln = axes.plot(df2[i].y,np.log(df2[i].num_exporters),color=colors[i],alpha=0.7,label=slabs[i])
    lns=lns+ln
...
axes.axvline(NU,color='black',linestyle='--',linewidth=1,alpha=0.7)
#axes.set_xlim(1974,20)
axes.set_ylabel('Log num. firms')
ax2.set_ylabel('Avg. tariff')

labs = [l.get_label() for l in lns]
axes.legend(lns,labs,loc='lower right',prop={'size':6})
fig.subplots_adjust(hspace=0.2,wspace=0.25)
plt.savefig(outpath + 'simul_trans_nf_sens_det.pdf',bbox_inches='tight')
plt.close('all')

###################################################

df = [df_[(df_.y>=1974)&(df_.y<=2017)].reset_index(drop=True) for df_ in df]
...
plt.savefig(outpath + 'tpu_regs_sens_det.pdf',bbox_inches='tight')
plt.close('all')
